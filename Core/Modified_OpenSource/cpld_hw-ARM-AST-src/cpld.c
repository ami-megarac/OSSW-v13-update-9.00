...
#define JTAG_MINOR           0
#define JTAG_MAX_DEVICES     1
#define JTAG_DEV_NAME        "jtag-altera"
...
static long altera_ioctl(struct file *file,unsigned int cmd, unsigned long arg)
{
  	...
  	switch (cmd)
	{
		...
		case IOCTL_JTAG_ERASE_JBC:
    	{
            ...
            if(Kernal_IO_Data.size < AST_FW_BUFFER_SIZE)
            {
                ...
                /* Reason for False Positive - jtag id can either be 0 or 1. So it doesn't result in negative number. */
	/* coverity[ underflow : FALSE ] */
            ...
            {
                /* Reason for False Positive - It is a debug print, so it doesn't result in negative number. */
	/* coverity[ underflow : FALSE ] */
