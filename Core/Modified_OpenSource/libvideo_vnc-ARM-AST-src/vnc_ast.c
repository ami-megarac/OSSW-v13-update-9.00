...
int full_frame_counter = 0, isNewSlaveSession = 0; 
...
int captureVideo( void **frame_hdr,void **video_data) {
	...
	if ( isNewSession || isNewSlaveSession) {
		...
		isNewSession = FALSE;
		if(isNewSlaveSession){ //Slave session will not be updated when there is no change in host video, so we need to send multiple full frames to update video in slave sesion 
			full_frame_counter++;
			if(full_frame_counter > MAX_FULL_FRAME_COUNT){
				isNewSlaveSession = FALSE; 
