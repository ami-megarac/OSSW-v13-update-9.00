...
int capture_video(void **video_data){
	VIDEO_Ioctl_AVICAA 	mioc={0};
	int 			capture_status;
	u16* 			p_primary_buf;
	//u16*                    p_comp_buf_dest;
...
//	u32 data_buf_size=0, frame_hdr_size=0;
	...
	capture_status = capture_video (&p_data_buf);
	...
	/*Reason for false positive - "tiltecount" is a intiger data type only we need integer data not need for explicit type cast to double*/
	/*coverity[integer_division : FALSE]*/
