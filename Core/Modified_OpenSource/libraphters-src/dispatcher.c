...
void dispatch() {
	...
	char *uri_info = get_request_uri();
	if (uri_info == NULL) {
		error_handler("NULL uri_info");
		return;
	}
	
	wRet = regcomp(&regexext, "^/ext/", 0);
	if(wRet != 0)
	{
		return;		
	}
	wRetExt = regexec(&regexext, uri_info, 1, match, 0);
    regfree(&regexext);

	wRet = regcomp(&regexapi, "^/api/", 0);
	if(wRet != 0)
	{
		return;
	}
	wRetApi = regexec(&regexapi, uri_info, 1, match, 0);
	regfree(&regexapi);
	if(wRetApi != 0 && wRetExt != 0)
	{
		path_info[0] = '\0';
	}
