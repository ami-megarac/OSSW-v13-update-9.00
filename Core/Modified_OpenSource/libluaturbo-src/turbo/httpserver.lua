...
function httpserver.HTTPConnection:_on_headers(data)
    ...
    if self.kwargs.read_body ~= false then
        ...
        if content_length then
            ...
            local header_checker = self.kwargs.header_checker
            if type(header_checker) == "function" then
                header_checker(self._request)
                if self.stream:closed() then return end
            end
            ...
            if type(self.kwargs.streaming_multipart_bytes) == "number" and
                ...
                content_type ~= nil and content_type:find("multipart/form-data", 1, true) then
