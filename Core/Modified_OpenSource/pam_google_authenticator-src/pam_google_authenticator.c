...
static void log_message(int priority, pam_handle_t *pamh, const char *format, ...) {
    char *service = NULL;
    if (pamh) pam_get_item(pamh, PAM_SERVICE, (void *)&service);
    if (!service) service = "";
...
static const char *get_user_name(pam_handle_t *pamh, const Params *params) {
    ...
    if (params->debug) {
        log_message(LOG_INFO, pamh, "debug: start of google_authenticator for \"%s\"",
                    username);
    }
    return username;
}
...
static const char *get_rhost(pam_handle_t *pamh, const Params *params) {
    ...
    if (params->debug) {
        log_message(LOG_INFO, pamh, "debug: google_authenticator for host \"%s\"", (const char *)rhost);
    }
...
static char *get_secret_filename(pam_handle_t *pamh, const Params *params,
    ...
    // for the secret file.
    const char *spec =
        params->secret_filename_spec ? params->secret_filename_spec : SECRET;
    ...
    if (!params->fixed_uid) {
        ...
        *uid = -1;
        if (buf == NULL) {
            ...
            /* Reason: variables will be finalized or freed in label "errout". */
    ...
    int allow_tilde = 1;
    int offset;
    for (offset = 0; secret_filename[offset];) {
...
// First, look up the user's default group
    ...
    if (gid_o < 0 && (gid_o = setgroup(gid)) < 0) {
        ...
        log_message(LOG_ERR, pamh, "Failed to change group id for user \"%s\" to %d",
                    username, (int)gid);
        return -1;
    }
...
static int open_secret_file(pam_handle_t *pamh, const char *secret_filename,
                            struct Params *params, const char *username, int uid,
                            struct stat *orig_stat) {
    // Try to open "~/.google_authenticator"
    int fd = open(secret_filename, O_RDONLY);
    if (fd < 0 || fstat(fd, orig_stat) < 0) {
        if (params->nullok != NULLERR && errno == ENOENT) {
            ...
            log_message(LOG_ERR, pamh, "Failed to read \"%s\" for \"%s\"",
    ...
    if (orig_stat->st_mode & 03777 & ~params->allowed_perm) {
        log_message(LOG_ERR, pamh,
                    ...
                    secret_filename, orig_stat->st_mode & 03777, params->allowed_perm);
        goto error;
    }
    ...
    if (!params->no_strict_owner && (orig_stat->st_uid != (uid_t)uid)) {
        ...
        log_message(LOG_ERR, pamh, "Secret file \"%s\" must be owned by \"%s\"",
                    secret_filename, username);
        goto error;
    }
    ...
    if (orig_stat->st_size < 1 || orig_stat->st_size > 64 * 1024) {
        log_message(LOG_ERR, pamh, "Invalid file size for \"%s\"", secret_filename);
        goto error;
    }
...
static char *read_file_contents(pam_handle_t *pamh, const Params *params,
                                const char *secret_filename, int *fd, off_t filesize) {
    ...
    *fd = -1;
    ...
    if (memchr(buf, 0, filesize)) {
        log_message(LOG_ERR, pamh, "Invalid file contents in \"%s\"", secret_filename);
        goto out;
    }
...
static int is_totp(const char *buf) { return !!strstr(buf, "\" TOTP_AUTH"); }
...
// Return 0 on success, errno otherwise.
static int write_file_contents(pam_handle_t *pamh, const Params *params,
                               const char *secret_filename, struct stat *orig_stat,
                               const char *buf) {
    int err = 0;
    int fd = -1;
    ...
    if (fnlength - 1 !=
        (unsigned int)snprintf(tmp_filename, fnlength, "%s~XXXXXX", secret_filename)) {
        err = ERANGE;
        goto cleanup;
    }
    ...
    if (fd < 0) {
        err = errno;
        log_message(LOG_ERR, pamh, "Failed to create tempfile \"%s\": %s", tmp_filename,
                    strerror(err));
    ...
    // `rename` below)
    {
        ...
        if (sb.st_ino != orig_stat->st_ino || sb.st_size != orig_stat->st_size ||
...
static uint8_t *get_shared_secret(pam_handle_t *pamh, const Params *params,
                                  const char *secret_filename, const char *buf,
                                  int *secretLen) {
    if (!buf) {
        return NULL;
    }
...
static char *get_cfg_value(pam_handle_t *pamh, const char *key, const char *buf) {
...
static int set_cfg_value(pam_handle_t *pamh, const char *key, const char *val,
    ...
    if (total_len <= (unsigned int)(stop - start)) {
    ...
    for (line = start + 4 + key_len + val_len; *line;) {
        char *ptr;
        if (line[0] == '"' && line[1] == ' ' && !strncmp(line + 2, key, key_len) &&
            (!*(ptr = line + 2 + key_len) || *ptr == ' ' || *ptr == '\t' ||
             *ptr == '\r' || *ptr == '\n')) {
...
static int step_size(pam_handle_t *pamh, const char *secret_filename, const char *buf) {
    ...
    if (errno || !*value || value == endptr ||
        ...
        step < 1 || step > 60) {
        free((void *)value);
        log_message(LOG_ERR, pamh, "Invalid STEP_SIZE option in \"%s\"", secret_filename);
        return 0;
    }
    free((void *)value);
    return step;
...
static int rate_limit(pam_handle_t *pamh, const char *secret_filename, int *updated,
                      char **buf) {
    ...
    if (((attempts = (int)strtoul(ptr = endptr, (char **)&endptr, 10)) < 1) ||
        ptr == endptr || attempts > 100 || errno || (*endptr != ' ' && *endptr != '\t') ||
        ((interval = (int)strtoul(ptr = endptr, (char **)&endptr, 10)) < 1) ||
        ptr == endptr || interval > 3600 || errno) {
        free((void *)value);
    ...
    while (*endptr && *endptr != '\r' && *endptr != '\n') {
        ...
        errno = 0;
        if ((*endptr != ' ' && *endptr != '\t') ||
            ((timestamp = (int)strtoul(ptr = endptr, (char **)&endptr, 10)), errno) ||
            ptr == endptr) {
            free((void *)value);
            free(timestamps);
            log_message(LOG_ERR, pamh,
                        "Invalid list of timestamps in RATE_LIMIT. "
                        "Check \"%s\".",
                        secret_filename);
            return -1;
        }
        num_timestamps++;
        unsigned int *tmp =
            (unsigned int *)realloc(timestamps, sizeof(int) * num_timestamps);
        if (!tmp) {
            free(timestamps);
            goto oom;
        }
    ...
    *updated = 1;
    ...
    if (exceeded) {
        log_message(LOG_ERR, pamh,
                    "Too many concurrent login attempts (\"%s\"). Please try again.",
                    secret_filename);
        return -1;
    }
...
static void conv_error(pam_handle_t *pamh, const char *text) {
    PAM_CONST struct pam_message msg = {
        .msg_style = PAM_ERROR_MSG, .msg = text,
    };
    PAM_CONST struct pam_message *msgs = &msg;
    struct pam_response *resp = NULL;
    const int retval = converse(pamh, 2, &msgs, &resp);
    if (retval != PAM_SUCCESS) {
 ...
 * applied.
 */
static int check_scratch_codes(pam_handle_t *pamh, const Params *params,
                               const char *secret_filename, int *updated, char *buf,
                               int code) {
    ...
    for (;;) {
        ...
        // that line.
        if (errno || ptr == endptr || (*endptr != '\r' && *endptr != '\n' && *endptr) ||
        ...
        if (scratchcode == code) {
            ...
            *updated = 1;
            ...
            if (params->debug) {
                log_message(LOG_INFO, pamh,
                            "debug: scratch code %d used and removed from \"%s\"", code,
                            secret_filename);
            }
            return 0;
        }
...
static int window_size(pam_handle_t *pamh, const char *secret_filename, const char *buf) {
...
static int invalidate_timebased_code(int tm, pam_handle_t *pamh,
    ...
    // Add the current timestamp to the list of disallowed timestamps.
    {
        ...
        if (!resized) {
            free((void *)disallow);
            log_message(LOG_ERR, pamh, "Failed to allocate memory when updating \"%s\"",
                        secret_filename);
            return -1;
        }
        disallow = resized;
 ...
 */
static int check_time_skew(pam_handle_t *pamh, int *updated, char **buf, int skew,
                           int tm) {
    int rc = -1;
    ...
    if (resetting) {
        ...
        // more times.
        if (num_entries &&
            (unsigned int)(tm + skew) ==
                (unsigned int)(tms[num_entries - 1] + skews[num_entries - 1])) {
            free((void *)resetting);
            return -1;
        }
    }
    ...
    if (num_entries == sizeof(tms) / sizeof(int)) {
        ...
        unsigned int i;
        for (i = 1; i < sizeof(tms) / sizeof(int); ++i) {
            ...
            if (tms[i] <= last_tm || tms[i] > last_tm + 2 || last_skew - skew < -1 ||
                last_skew - skew > 1) {
                goto keep_trying;
            }
            last_tm = tms[i];
        ...
        *reset = '\000';
        if (rc) {
            for (i = 0; i < num_entries; ++i) {
                ...
                snprintf(pos, reset_size - (pos - reset), " %d%+d" + !*reset, tms[i],
                         skews[i]);
            }
        }
 ...
 */
int update_logindetails(pam_handle_t *pamh, const Params *params, char **buf) {
 ...
 */
int within_grace_period(pam_handle_t *pamh, const Params *params, const char *buf) {
    const char *rhost = get_rhost(pamh, params);
    const time_t now = get_time();
    ...
    return (when + (unsigned long)(grace > now));
}
 ...
 */
static int check_counterbased_code(pam_handle_t *pamh, const char *secret_filename,
                                   int *updated, char **buf, const uint8_t *secret,
                                   int secretLen, int code, long hotp_counter,
...
static int google_authenticator(pam_handle_t *pamh, int argc, const char **argv) {
    ...
    // Drop privileges.
    {
        ...
        if (uid == -1) {
            ...
            if (!secret_filename) {
                ...
                /* Reason: variables will be finalized or freed in label "out". */
                /* coverity[name_at_decl_position: FALSE] */
                goto out;
            }
        }
    ...
    if (secret_filename) {
        ...
        if (buf) {
            if (rate_limit(pamh, secret_filename, &early_updated, &buf) >= 0) {
                secret =
                    get_shared_secret(pamh, &params, secret_filename, buf, &secretLen);
            } else {
    ...
    if (buf && within_grace_period(pamh, &params, buf)) {
        rc = PAM_SUCCESS;
        log_message(LOG_INFO, pamh, "within grace period: \"%s\"", username);
        goto out;
    }

    // Only if nullok and we do not have a code will we NOT ask for a code.
    // In all other cases (i.e "have code" and "no nullok and no code") we DO ask for a
    // code.
    if (!stopped_by_rate_limit && (secret || params.nullok != SECRETNOTFOUND)) {
        if (!secret) {
            log_message(LOG_WARNING, pamh,
                        "No secret configured for user %s, asking for code anyway.",
                        username);
        }
        ...
        for (mode = 0; mode < 4; ++mode) {
                ...
                default:
                    if (params.pass_mode == PROMPT ||
                        ...
                        if (!saved_pw) {
                            ...
                            // we cannot tell the difference between an eight digit
                            // scratch code or a two digit password immediately followed
                            // by a six digit verification code. We have to loop and try
                            // both options.
            ...
            if (secret) {
                ...
                switch (check_scratch_codes(pamh, &params, secret_filename, &updated, buf,
                                            code)) {
                    case 1:
                        if (hotp_counter > 0) {
                            switch (check_counterbased_code(pamh, secret_filename, &updated,
                                                            &buf, secret, secretLen, code,
                                                            hotp_counter, &must_advance_counter)) {
                                case 0:
                                    rc = PAM_SUCCESS;
                                    break;
                                case 1:
                            ...
                            switch (check_timebased_code(pamh, secret_filename, &updated,
                                                         &buf, secret, secretLen, code,
                                                         &params)) {
                                case 0:
                                    rc = PAM_SUCCESS;
        ...
        if (rc == PAM_SUCCESS) {
            ...
            if (params.grace_period != 0) {
                if (update_logindetails(pamh, &params, &buf)) {
                    log_message(LOG_ERR, pamh,
                                "Failed to store grace_period timestamp in config");
                }
            }
        } else {
    ...
    if (early_updated || updated) {
        int err;
        if ((err =
                 write_file_contents(pamh, &params, secret_filename, &orig_stat, buf))) {
            ...
            switch (err) {
                ...
                case EIO:
                case EDQUOT:
                    snprintf(s, sizeof(s), "Error \"%s\" while writing config",
                             strerror(err));
                    conv_error(pamh, s);
                    break;
                default:
...
out:
    if (params.debug) {
        log_message(LOG_INFO, pamh,
                    "debug: end of google_authenticator for \"%s\". Result: %s", username,
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags UNUSED_ATTR, int argc,
                                   const char **argv) {
    return google_authenticator(pamh, argc, argv);
}

PAM_EXTERN int pam_sm_setcred(pam_handle_t *pamh UNUSED_ATTR, int flags UNUSED_ATTR,
                              int argc UNUSED_ATTR, const char **argv UNUSED_ATTR) {
    return PAM_SUCCESS;
}
