...
std::array<std::tuple<std::string, std::string, std::string>, 6> bdfs = {
    std::make_tuple("0", "0", "B02_D0a_F3_"), std::make_tuple("0", "1", "B02_D0a_F7_"),
    std::make_tuple("0", "2", "B02_D0b_F3_"), std::make_tuple("1", "0", "B02_D0c_F3_"),
    std::make_tuple("1", "1", "B02_D0c_F7_"), std::make_tuple("1", "2", "B02_D0d_F3_")
};
...
bool decodeSingleRetryRd(std::stringstream& ssKey, std::stringstream& ssValue, json silkscreenMap,
                         RetryRdSet decodedRetryRd, std::string socketId, std::string imc, std::string channel)
{
...
void decodeImcRegisters(const json& input, json& memoryErrors, json silkscreenMap, std::string socketId)
{
    ...
    for (const auto& [entry, value] : input["uncore_status_regs"][socket].items())
    {
        ...
        if (!(str2uint(std::string(value), decodedRetryRd.errLog.data) && str2uint(input["uncore_status_regs"][socket][std::string(entry + "_address1")],
                        decodedRetryRd.errLogAddress1.data) && str2uint(input["uncore_status_regs"][socket][std::string(entry + "_address2")],
...
void decodeBdfRegisters(const json& input, json& memoryErrors, json silkscreenMap, std::string socketId)
{
    ...
    for (const auto& [imcNumber, channelNumber, registerBdf] : bdfs)
    {
        ...
        if (!(input["uncore_status_regs"][socket].contains(retryRdErrLogOffset) && input["uncore_status_regs"][socket].contains(retryRdErrLogAddress1offset) &&
...
json decodeRetryLog(const json& input, json silkscreenMap, std::string socketId)
{
    json memoryErrors;
...
json decodeRetryLog(const json& input, json silkscreenMap, std::string socketId)
{
    ...
    for (const auto& [imc, channel, registerBdf] : bdfs)
    {
        if (!readRetryRd(decodedRetryRd, registerBdf, input))
        {
            continue;
        }
        ...
        if (!decodeSingleRetryRd(ssKey, ssValue, silkscreenMap, decodedRetryRd, socketId, imc, channel))
        {
            continue;
        }
...
std::array<std::pair< std::string, std::string>, 4> imcBdfs = {
    std::make_pair("IMC 0", "B30_D26_F0_"), std::make_pair("IMC 1", "B30_D27_F0_"),
    std::make_pair("IMC 2", "B30_D28_F0_"), std::make_pair("IMC 3", "B30_D29_F0_")
};
...
std::array <RetryRdOffsets, 2> setOffsets = {RetryRdOffsets{"C60", "C58", "C28", "ED8"}, RetryRdOffsets{"E54", "E58", "E5C", "EE0"}};
                         ...
                         RetryRdSet decodedRetryRd, std::string socketId, std::string imc, std::string channel)
{
    ...
    std::string dimm =
        std::to_string(static_cast<uint8_t>(decodedRetryRd.errLogAddress1.chip_select >> 2 & 0x1));
...
bool readRetryRd(RetryRdSet& decodedRetryRd, std::string registerBdf,
                RetryRdOffsets registersSetOffsetsBegin, RetryRdOffsets registersSetOffsetsEnd, const json& input)
{
    std::string retryRdErrLogOffset = registerBdf + registersSetOffsetsBegin.errLog + registersSetOffsetsEnd.errLog;
    std::string retryRdErrLogAddress1offset = registerBdf + registersSetOffsetsBegin.errLogAddress1 + registersSetOffsetsEnd.errLogAddress1;
    std::string retryRdErrLogAddress2offset = registerBdf + registersSetOffsetsBegin.errLogAddress2 + registersSetOffsetsEnd.errLogAddress2;
    std::string retryRdErrLogAddress3offset = registerBdf + registersSetOffsetsBegin.errLogAddress3 + registersSetOffsetsEnd.errLogAddress3;
...
json decodeRetryLog(const json& input, json silkscreenMap, std::string socketId)
{
    ...
    for (const auto& [imc, registerBdf] : imcBdfs)
    {
        for (const auto& [channel, registersSetOffsetsBegin] : channelOffsets)
        {
            for (const auto& registersSetOffsetsEnd : setOffsets)
            {
                if (!readRetryRd(decodedRetryRd, registerBdf, registersSetOffsetsBegin,registersSetOffsetsEnd, input))
                {
                    continue;
                }
    ...
    parseMemoryErrors(correctableErrors, uncorrectableErrors, memoryErrors);
...
std::array <RetryRdOffsets, 3> setOffsets = {RetryRdOffsets{"c60", "c58", "c28", "ed8"}, 
    RetryRdOffsets{"e54", "e58", "e5c", "ee0"}, RetryRdOffsets{"c70", "d58", "c64", "f10"}};
