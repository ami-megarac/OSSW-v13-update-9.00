#!/usr/bin/luajit
--[[
Ref. lj2curl library (https://github.com/Wiladams/lj2curl)
--]]
...
function EZreq.init()
	...
	ffi.gc(handle, curl.curl_easy_cleanup)
	local newobj = {
		Handle = handle
	}
	setmetatable(newobj, EZreq_mt)
	return newobj
end

function EZreq.setOption(self, opt, value)
	local param
	local t_value = type(value)
	if t_value == "number" then
		param = ffi.new("long", value)
	elseif t_value == "string" then
		param = ffi.cast("const char *", value)
	elseif t_value == "cdata" then
		param = value
	else
		error("setOption value need one of type: number/string/cdata: " .. tostring(value))
	end
	local res = curl.curl_easy_setopt(self.Handle, opt, param)
	if res ~= C.CURLE_OK then
		return false, tonumber(res), ffi.string(curl.curl_easy_strerror(res));
	end
...
function EZreq.getInfo(self, info)

	local value = ffi.new("curl_info_union_t")
	local res = curl.curl_easy_getinfo(self.Handle, info, value);
	if C.CURLE_OK ~= res then
		return false, tonumber(res), ffi.string(curl.curl_easy_strerror(res));
	end

	local infoType = band(C.CURLINFO_TYPEMASK, info)

--print(string.format("\ninfoType: 0x%x", infoType))
	if infoType == C.CURLINFO_STRING then
		return ffi.string(value.strValue)
	elseif infoType == C.CURLINFO_LONG then
		return tonumber(value.longValue)
	elseif infoType == C.CURLINFO_DOUBLE then
