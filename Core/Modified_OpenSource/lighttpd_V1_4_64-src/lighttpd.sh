...
if [ -x /usr/local/sbin/lighttpd ]; then
    if ! grep '"mod_proxy"' $lighttpd_file >> /dev/null; then
        echo 'server.modules += ( "mod_proxy" )' >> $lighttpd_file
    fi
    if ! grep 'h5sol' $lighttpd_file >> /dev/null; then
        echo '$HTTP["url"] =~ "^/h5sol" {' >> $lighttpd_file
        echo '    proxy.server = ( "" => ( ( "host" => "127.0.0.1", "port" => "2371" ) ) )' >> $lighttpd_file
        echo '    proxy.header = ( "upgrade" => "enable" )' >> $lighttpd_file
        echo '	setenv.set-response-header = ( "Sec-WebSocket-Protocol" => "binary" )' >> $lighttpd_file
        echo '}' >> $lighttpd_file
    fi
