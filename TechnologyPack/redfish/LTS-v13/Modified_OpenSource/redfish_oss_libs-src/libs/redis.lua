...
-- Command pipelining
client_prototype.pipeline = function(client, block, transaction)
    local transaction = transaction ~= false
...
if transaction then
    local multi_ret = pipeline:multi()
end
    local success, retval = pcall(block, pipeline)
if transaction then
    local exec_ret = pipeline:exec()
end
...
if transaction then
    local raw_replies = replies[#replies]
    replies = {}
    for i = 2, #requests-1 do
        local reply, parser = raw_replies[i-1], parsers[i]
        if parser then
            reply = parser(reply)
        end
        table_insert(replies, i-1, reply)
    end
end
